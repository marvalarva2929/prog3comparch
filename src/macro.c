...
Entry createExpandedEntry(Entry *original, const char *instruction, int addressOffset)
{
	...
	if (strcmp(cmd, "xor") == 0)
		entry.cmd.type = XOR;
	else if (strcmp(cmd, "priv") == 0)
		entry.cmd.type = PRIV;
	else if (strcmp(cmd, "subi") == 0)
		entry.cmd.type = SUBI;
	else if (strcmp(cmd, "addi") == 0)
		entry.cmd.type = ADDI;
	else if (strcmp(cmd, "mov") == 0)
		entry.cmd.type = MOV;
	else if (strcmp(cmd, "shftli") == 0)
		entry.cmd.type = SHFTLI;
	else
	{
...
int expandLd(Entry *original, Entry *output, uint64_t addr)
{
	...
	sprintf(instruction, "xor r%i, r%i, r%i", 1, 1, 1);
	printf("penis4\n %s", instruction);
